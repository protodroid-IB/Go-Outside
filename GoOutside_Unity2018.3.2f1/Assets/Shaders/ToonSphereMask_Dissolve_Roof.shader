Shader "Custom/Toon_SphereMask_Dissolve_Roof"
{
    Properties
    {
		...
		[MaterialToggle]_DisableDissolve("Disable Dissolve", float) = 0
    }
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
			...
			{
				test = tex2D(_DissolveTex, IN.uv_MainTex).rgb - (_DissolveAmount);
				clip(test);
			...
			{
				float3 primaryTex = (step(sphereNoise - _DissolveLineWidth, _DissolveAmount) * c2.rgb);
				secondaryTex = (step(_DissolveAmount, sphereNoise) * c.rgb);
				resultTex = primaryTex + secondaryTex + DissolveLine;

				test = tex2D(_DissolveTex, IN.uv_MainTex).rgb - (10.0 * sphere);
				clip(-test);
